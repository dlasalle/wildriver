...
static void splitTest()
{
  ...
  std::vector<std::string> chunks = Util.split(tripleLine);
  ...
  std::vector<std::string> chunks = Util.split(spaceAndTabs);
  ...
  std::vector<std::string> chunks = Util.split(trailingAndLeading);
  testEquals(chunks.size(), 2);
  testEquals(chunks[0].compare("hello"), 0);
  testEquals(chunks[1].compare("world"), 0);
}
